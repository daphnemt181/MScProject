...
from recognition_models import RecMLP as RecognitionModel
...
def main(args):
    ...
    if load_param_dir:
        ...
        out_dir = os.path.join(PATHS['save_dir'], 'mlp_wavenet_translation_fr_en.{}'.format(timestamp))
    ...
    # valid vocabulary
    processed_data_path = os.path.join(PATHS['data_processed_dir'], 'fr_en_2to30_vocab30000_un_europarl_nc')
    ...
    pp_num_datapoints = args.pp_num_datapoints
    pp_batch_size = args.pp_batch_size
    pp_num_samples = args.pp_num_samples
    ...
    if translate:
        ...
        run.translate(num_output=200, translation_source='x', word_dict_x=valid_vocab_en, word_dict_y=valid_vocab_fr)
        print("         TRANSLATING FROM FRENCH TO ENGLISH")
        run.translate(num_output=200, translation_source='y', word_dict_x=valid_vocab_fr, word_dict_y=valid_vocab_en)
    ...
    if perplexity:
        run.perplexity(num_datapoints=pp_num_datapoints, batch_size=pp_batch_size, num_samples=pp_num_samples)

        print('='*70)
        print('         COMPLETED PERPLEXITY')
        print('='*70)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--perplexity",
                        dest="perplexity",
                        help="If we are to do perplexity phase as well",
                        default=1,
                        type=int)
    parser.add_argument("--pp_num_datapoints",
                        dest="pp_num_datapoints",
                        help="Number of datapoints to calculate perplexity over",
                        default=0,
                        type=int)
    parser.add_argument("--pp_batch_size",
                        dest="pp_batch_size",
                        help="Batch size used for perplexity",
                        default=2p00,
                        type=int)
    parser.add_argument("--pp_num_samples",
                        dest="pp_num_samples",
                        help="Number of samples to use when calculating perplexity",
                        default=1,
                        type=int)
