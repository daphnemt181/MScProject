...
def get_probs(chars, trans_probs):
    """Get probability of a sequence of characters

    Given a transition table (canvas) get the probability of
    a sequence of characters that has been generated by the model.

    :param chars: N * max(L) matrix, N is the number of sentences,
    max(L) is the number of characters in the longest sentence
    :param trans_probs: N * max(L) * D * D tensor, canvas of the transition
    probabilities
    :return: probabilities as a theano tensor
    """
    ...
    # returns the probability sequence for all N trajectories
    return probs.T


def sample(trans_probs, num_samples):
    """Generate num_samples samples using trans_probs

    Using trans_probs tensor as the Markov chain generate
    num_samples samples from the implied probability distribution
    ...
    # repeat the trans_probs markov chain num_samples times in order to
    # sample many times from the probability distribution
    ...
    def step(trans_probs_l, chars_lm1):
        """Generate the next character given the last samples character"""
        # get the relevant probability distributions using the rows from the
        # transition tensor trans_probs_l indexing by previously samples characters
        probs_l = trans_probs_l[T.arange(trans_probs_l.shape[0]), T.cast(chars_lm1, 'int32')]  # (S*N) * D
        # sample one-hot next characters
        chars_l_one_hot = random.multinomial(pvals=probs_l)  # (S*N) * D
        # recast back into usual form (not the one-hot vector form)
