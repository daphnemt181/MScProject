"""
Autoencoder example for AEVB paper
"""
...
class VariationalAutoencoder(object):
    def __init__(self,
        ...
        # Encoder part (phi)
        self.encoder = utils.EncoderMLP(self.srng, input_tensor, n_in, hdim, ldim)
        # Decoder part (theta)
        ...
        # Cost function
        self.kl_div = T.mean(utils.kl_unit_normal(self.encoder.mu, self.encoder.sigma2))
...
def main(args):
    ...
    auto_nn = VariationalAutoencoder(args.learning_rate,
                                     args.hdim,
                                     args.ldim,
                                     X,
                                     n_in=28 * 28,
    ...
    # Load data
    train_set, val_set, test_set = utils.load_data('mnist.pkl.gz')
        ...
        if epoch % args.eval_frequency == 0:
            ...
            if val_cost < best_validation_loss:
                ...
                with open(os.path.join(save_path, 'best_model.pkl'), 'wb') as f:
                    pickle.dump(auto_nn.predict, f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    predicted = auto_nn.predict(test_set[index, :])
    # Baseline images
